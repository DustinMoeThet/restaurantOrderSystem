<?php

namespace App\Http\Controllers;

use App\Models\Order;
use App\Models\Category;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class CategoryController extends Controller
{
    //direct category list page
    public function list(){
        $categories = Category::when(request('key'),function($query){
            $query->where('name','like','%'.request('key').'%');
        })
        ->orderBy('id')->paginate(4);
        $hasPendingOrders = $this->hasPendingOrders();
        return view('admin.category.list',compact('categories','hasPendingOrders'));
    }

    //direct category create page
    public function createPage(){
        $hasPendingOrders = $this->hasPendingOrders();
        return view('admin.category.create',compact('hasPendingOrders'));
}

    //category creation

    public function create(Request $request){
        $this->categoryValidationCheck($request);
        $data = $this->requestCategoryData($request);
        Category::create($data);
        return redirect()->route('category#list');
    }

    //category deletion
    public function delete($id){
        Category::where('id',$id)->delete();
        return back()->with(['deleteSuccess'=>'Deletion Success']);
    }


    //category edit

    public function edit($id){
        $category=Category::where('id',$id)->first();
        $hasPendingOrders = $this->hasPendingOrders();
        return view('admin.category.edit',compact('category','hasPendingOrders'));
    }

    //category update
    public function update($id,Request $request){
        $this->categoryUpdateValidationCheck($request,$id);
        $data = $this->requestCategoryData($request);
        Category::where('id',$id)->update($data);
        return redirect()->route('category#list');
    }

    //private functions

    //category validation check

    private function categoryValidationCheck($request){
        Validator::make($request->all(),[
            'categoryName'=>'required|min:4|unique:categories,name,'
        ])->validate();
    }

    //update validation check
    private function categoryUpdateValidationCheck($request,$id){
        Validator::make($request->all(),[
            'categoryName'=>'required|min:4|unique:categories,name,'.$id
        ])->validate();
    }

    // request category data

    private function requestCategoryData($request){
        return [
            'name'=>$request->categoryName
        ];
    }

    private function hasPendingOrders()
    {
        $pendingOrdersCount = Order::where('status', '0')->count();
        return $pendingOrdersCount > 0;
    }
}
